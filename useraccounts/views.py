...
def userprofile(request):
    if request.method == "POST" and 'amount' in request.POST and 'remarks' in request.POST:
        ...
        newDiet = diet(messname=messname,
                       customer=customer,
                       remarks=remarks,
                       amount=amount)
        newDiet.save()
        try:
            cexist = credittable.objects.filter(messname=messname,
                                                customer=customer)
            if len(cexist) > 0:
                chkval = 1
            else:
        ...
        if chkval == 0:
            ccreate = credittable(messname=messname,
                                  customer=customer,
                                  credit=0)
            ccreate.save()
...
def sellerrecords(request):
    ...
    return render(request, "sellerrecords.html", {
        "customer": False,
        "messname": messname.messcanteen,
        "diets": diets
    })
...
def profilesettings(request):
    ...
    if profiletype.customer == False:
        try:
            ...
            return render(request, "profilesettingsprofile.html", {
                "customer": profiletype.customer,
                "mess": usermess.messcanteen
            })
        except:
            return render(request, "profilesettingsprofile.html",
                          {"customer": profiletype.customer})
...
def emailsettings(request):
    ...
    if request.method == "POST":
        ...
        user.email = email
        user.save()
        return render(request, "profilesettingsemail.html",
...
def loginajax(request):
    ...
    data = {'user': User.objects.filter(username__iexact=username).exists()}
...
def viewdetailsmesswise(request):
    ...
    return render(request, "messdetails.html", {
        "diets": diets,
        "messname": mess
    })
    ...
    if request.method == "POST":
        ...
        try:
            ...
            payload = {
                "From": "LOBIFY",
                "To": str(mob_num),
                "TemplateName": "usercredit",
                "VAR1": str(request.user.username),
                "VAR2": str(amount),
                "VAR3": str(link)
            }
            response = requests.request("POST", url, data=payload)
